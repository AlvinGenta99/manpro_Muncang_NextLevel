<?php
/* Search function and display for Recycle Page.
...
<head>
 <h1><center>Recycle Log<center></h1>
</head>
...
?>
    <nav class="nav nav-pills" id = "left-panel-link">
        ...
        <a href="insert_recycle.php" class="nav-item nav-link active">
...
<br>
  ...
  <?php
	...
	//Search query
    if ($select_by == 'recycle_count')
    {
        $query = @pg_query("select * from view_recycle WHERE " . $select_by . " = " . $search . "") or @die(error);
    }
    else
    {
        $query = pg_query("select * from view_recycle WHERE " . $select_by . " LIKE '%" . $search . "%'") or die(error);
    }
...
?>
    <tr>                
        <td><?php echo $data['id_recycle']; ?></td>
        <td><?php echo $data['nama_tipe']; ?></td>
        <td><?php echo $data['recycle_count']; ?></td>                        
    </tr>
    <?php
    }
?>
...
<nav class="nav nav-pills" id = "left-panel-link">
    <a href="view_recycle.php" class="nav-item nav-link active">
